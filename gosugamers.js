var decklist = [];
$('[class^="card-link"]').each(function(i, el) {
	//$('.card-link card ').each (function(i, el) {
	var values = $('[class^="name card-quality"]', this).text(); //substring(2).substring(0, $(this).text().length - 10).replace(/[^a-zA-Z0-9\.\s']+/g ,"").split(' ');
...
fileName = $('h2 [class^="class-color"]').text() + '.txt';
